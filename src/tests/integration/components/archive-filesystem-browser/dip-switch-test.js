...
describe('Integration | Component | archive filesystem browser/dip switch', function () {
  setupComponentTest('archive-filesystem-browser/dip-switch', {
    integration: true,
  });
  ...
  it('displays DIP as active if archiveDipMode is "dip"', function () {
    ...
    this.render(hbs `{{archive-filesystem-browser/dip-switch
      archiveDipMode=archiveDipMode
    }}`);

    expect(this.$('.select-archive-dip-btn')).to.have.class('active');
  });
