...
import { providerEntityType } from 'oneprovider-gui/models/provider';
  ...
  providerNamesProxy: promise.array(
    computed('dbViewProxy.content.providers', function providerNamesProxy() {
      ...
      return this.get('dbViewProxy').then((dbView) => {
        ...
        const providersGris = providers.map((providerId) => gri({
          entityType: providerEntityType,
          entityId: providerId,
          aspect: 'instance',
          scope: 'private',
        }));
        return providersGris.map(gri =>
          store.findRecord('provider', gri)
          .then(providers => providers.mapBy('name'))
        );
      });
    }),
  ),
